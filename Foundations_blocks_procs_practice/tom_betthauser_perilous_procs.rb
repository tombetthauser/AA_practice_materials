...
# should return a boolean indicating whether or not all elements of the array 
...
# to the block come before all of the elements that return false when given to 
# the block. This means that the new array has two partitions; the order among 
...
# original word returns true when passed into the key proc. If an original word 
# returns true for multiple key procs, then the value proc changes should be 
...
# sentence where each word of the input sentence is changed by a value proc if the 
# original word returns true when passed into the key proc. If an original word 
# returns true for multiple key procs, then only the value proc that appears 
# earliest in the hash should be applied.

def picky_procipher(sentence, hash)
  keys_arr = hash.keys.reverse

  new_words_arr = sentence.split(" ")

  new_words_arr.each_with_index do |word, idx|
    keys_arr.each do |key|
      if key.call(word)
        new_words_arr[idx] = hash[key].call(word)
      end
    end
  end
